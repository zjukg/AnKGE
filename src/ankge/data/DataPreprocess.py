...
from .GetNearNeighbor import GetNearNeighbor
from .GetLevelScore import GetLevelScore
...
class KGData(object):
    ...
    def get_hr2t_rt2h_from_train(self):
        ...
        for h, r, t in self.train_triples:
            ...
            self.rt2h_train[(r, t)].add(h) # ent_level
            self.ht2r_train[(h, t)].add(r) # rel_level
            self.t2hr_train[t].add((h,r))  # triple_level
            
        for h, r in self.hr2t_train:
            self.hr2t_train[(h, r)] = np.array(list(self.hr2t_train[(h, r)]))
        for r, t in self.rt2h_train:
            self.rt2h_train[(r, t)] = np.array(list(self.rt2h_train[(r, t)])) # ent_level
        for h, t in self.ht2r_train:
            self.ht2r_train[(h, t)] = np.array(list(self.ht2r_train[(h, t)])) # rel_level
        for t in self.t2hr_train:
            self.t2hr_train[t] = np.array(list(self.t2hr_train[t])) # triple_level
...
class RevSampler(KGData):
    ...
    def get_train(self):
        if self.args.calculate_knn:
            knn = GetNearNeighbor(self.args, self.train_triples)
            nearest_neighbor = knn.calculate_nearest_neighbor() #tensor [num_train, n1+n2+n3+n3]
            self.knn_train_triples = torch.cat((torch.tensor(self.train_triples), nearest_neighbor), dim=1) #tensor [num_train, 3+n1+n2+n3+n3]
            return self.knn_train_triples.tolist()
        else:
            return self.train_triples

    def get_valid(self):
        #if self.args.calculate_knn:
        #    cand = GetLevelScore(self.args, self.valid_triples, self.rt2h_train, self.hr2t_train, self.t2hr_train)
        #    level_candidate = cand.get_level_score() #tensor [num_valid, num_ent*3]
        #    self.level_valid_triples = torch.cat((torch.tensor(self.valid_triples), level_candidate), dim=1)
        #    return self.level_valid_triples.tolist()
        #else:
        return self.valid_triples

    def get_test(self):
        #if self.args.calculate_knn:
        #    cand = GetLevelScore(self.args, self.test_triples, self.rt2h_train, self.hr2t_train, self.t2hr_train)
        #    level_candidate = cand.get_level_score() #tensor [num_test, num_ent*3]
        #    self.level_test_triples = torch.cat((torch.tensor(self.test_triples), level_candidate), dim=1)
        #    return self.level_test_triples.tolist()
        #else:
        return self.test_triples
